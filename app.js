...
const preLoadfoods = () => {
  fetch("https://www.themealdb.com/api/json/v1/1/filter.php?c=Seafood")
    .then((res) => res.json())
    .then((data) => showPreloadedFoods(data));
};
preLoadfoods();

const showPreloadedFoods = (data) => {
  const meals = data.meals;
  const preloadedMealDiv = document.getElementById("preloaded-meals-row");
  for (let i = 0; i < 8; i++) {
    const meal = meals[i];
    const divOfPreloadedMeals = document.createElement("div");
    divOfPreloadedMeals.className =
      "col-xm-1 col-sm-1 col-md-3 p-3 d-flex justify-content-center";
    divOfPreloadedMeals.innerHTML = `<div>
    <a
      href="#meal-details-section"
      style="
    ...
    >
      <div
        ...
        class="card border-0 shadow cursor"
    ...
    `;
    preloadedMealDiv.appendChild(divOfPreloadedMeals);
  }
};
const searchMeal = () => {
